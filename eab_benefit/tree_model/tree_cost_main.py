...
from index_eab.eab_benefit.tree_model.tree_cost_utils import tree_cost_com
from index_eab.eab_benefit.tree_model.tree_cost_utils.tree_cost_loss import cal_mape, QError

from index_eab.eab_benefit.tree_model.tree_cost_dataset import PlanPairDataset, normalize, unnormalize
from index_eab.eab_benefit.tree_model.tree_cost_model import XGBoost, LightGBM, RandomForest
...
tree_cost_com.set_logger(args.runlog.format(args.exp_id))
...
tree_cost_com.summary_writer = SummaryWriter(args.logdir.format(args.exp_id))
tree_cost_com.summary_writer.add_text(
    "parameters",
...
# args.train_data_load = "/data/wz/index/index_eab/eab_benefit/tree_model/data/tpch/tree_tpch_cost_data_tgt_train.json"
with open(args.train_data_load, "r") as rf:
    train_data = json.load(rf)
# args.valid_data_load = "/data/wz/index/index_eab/eab_benefit/tree_model/data/tpch/tree_tpch_cost_data_tgt_valid.json"
...
# 1. feat: 1) cost(sum, wsum), 2) row(sum, wsum)
# 2. label: 1) ratio(plan1/plan2), 2) value: pair_diff,
#           3) raw(plan1), 4) cla(0-1).
train_set = PlanPairDataset(train_data, plan_num=args.plan_num, feat_chan=args.feat_chan,
                            feat_conn=args.feat_conn, label_type=args.label_type, cla_min_ratio=args.cla_min_ratio)
valid_set = PlanPairDataset(valid_data, plan_num=args.plan_num, feat_chan=args.feat_chan,
                            feat_conn=args.feat_conn, label_type=args.label_type, cla_min_ratio=args.cla_min_ratio)
logging.info(f"The characteristic of the data is: `{args.feat_chan}, {args.feat_conn}, {args.label_type}`.")
