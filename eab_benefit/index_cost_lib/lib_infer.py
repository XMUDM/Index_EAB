...
# [2, 1 , 1       , 1      , 1       , 5 , 1           , 1            ]
# [𝐼𝑡, 𝐼𝑜, log(𝑟𝑜𝑤𝑠), 𝑑𝑖𝑠𝑡_𝑓𝑟𝑎𝑐, 𝑁𝑈𝐿𝐿_𝑓𝑟𝑎𝑐, 𝑂𝑡, log(𝑐𝑎𝑟𝑑_est), log(𝑐𝑎𝑟𝑑_act)]
data_label = []
...
# Find the minimum and maximum number of the index optimizable operations
max_l = 0
min_l = 999
...
pad_data = torch.tensor(pad_data)  # zw: torch.Size([5335, 36, 12])
mask = torch.tensor(mask)  # zw: torch.Size([5335, 36])
label = torch.tensor(label)  # zw: 5335
