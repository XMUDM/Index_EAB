# -*- coding: utf-8 -*-
# @Project: index_test
# @Module: tpch_skew_multi_query_bench
# @Author: Wei Zhou
...
def get_heu_multi_query_res():
    ...
    # args.work_file = "/data1/wz/index/index_eab/eab_olap/bench_temp/tpch_skew/tpch_skew_1gb_template_22_multi500_query.json"
    # args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level/" \
    #                 f"tpch_skew_1gb_template_22_multi_query_index{start * gap}-{end * gap}.json"

    args.work_file = "/data1/wz/index/index_eab/eab_olap/bench_temp/tpch_skew/tpch_skew_query_temp_multi_c100_n1321_test.json"
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level/" \
                    f"tpch_skew_1gb_template_22_multi_query_index.json"
...
def get_swirl_multi_query_res():
    ...
    exp_dir = "/data1/wz/index/index_eab/eab_algo/swirl_selection/exp_res_main"
    rl_agents["swirl"] = [exp for exp in os.listdir(exp_dir) if
                          "swirl" in exp and "hskew_query" in exp and "_bak" not in exp]
    rl_agents["drlinda"] = [exp for exp in os.listdir(exp_dir) if "drlinda" in exp and "hskew_query" in exp]
    rl_agents["dqn"] = [exp for exp in os.listdir(exp_dir) if "dqn" in exp and "hskew_query" in exp]
    ...
    for agent in ["swirl"]:
        ...
        for instance in rl_agents[agent]:
            ...
            # db_conf["postgresql"]["db_name"] = "tpch_1gb103_skew"
        ...
        args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level/" \
                        f"tpch_skew_1gb_template_22_multi_query_index_{agent}.json"
        if args.res_save is not None:
            if not os.path.exists(os.path.dirname(args.res_save)):
                os.makedirs(os.path.dirname(args.res_save))
            with open(args.res_save, "w") as wf:
...
def get_mcts_multi_query_res():
    ...
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level/" \
                    f"tpch_skew_1gb_template_22_multi_query_index_mcts.json"
...
def get_mab_multi_query_res():
    ...
    #                 f"tpch_1gb_template_18_multi_work_index{start * gap}-{end * gap}.json"
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level/" \
                    f"tpch_skew_1gb_template_22_multi_query_index_mab.json"
...
def remove_step_info():
    res_dir = "/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/query_level"
    for file in tqdm(os.listdir(res_dir)):
        if "tpch_skew_1gb_template_22" not in file:
            continue
