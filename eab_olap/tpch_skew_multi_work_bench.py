# -*- coding: utf-8 -*-
# @Project: index_test
# @Module: tpch_skew_multi_work_bench
# @Author: Wei Zhou
...
def get_heu_multi_work_res():
    ...
    args.work_file = "/data1/wz/index/index_eab/eab_olap/bench_temp/tpch_skew/tpch_skew_work_temp_multi_w18_n100_test.json"
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_level/" \
                    f"tpch_skew_1gb_template_18_multi_work_sto5k_index{start * gap}-{end * gap}.json"

    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_level/" \
                    f"tpch_skew_1gb_template_18_multi_work_num5_index{start * gap}-{end * gap}.json"

    # args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_level/" \
    #                 f"tpch_skew_1gb_template_18_multi_work_sto900_index.json"

    args.varying_frequencies = True
    args.work_file = "/data1/wz/index/index_eab/eab_olap/bench_temp/tpch_skew/tpch_skew_work_temp_multi_w18_freq_n100_test.json"
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_freq/" \
                    f"tpch_skew_1gb_template_18_multi_work_freq_index{start * gap}-{end * gap}.json"
...
def get_swirl_multi_work_res(exp_id, res_id):
    ...
    args.work_file = "/data1/wz/index/index_eab/eab_olap/bench_temp/tpch_skew/tpch_skew_work_temp_multi_w18_permutation_n100_test.json"
    ...
    for agent in ["dqn"]:
        ...
        args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/{res_id}/" \
                        f"tpch_skew_1gb_template_18_multi_{res_id}_index_{agent}.json"
        if args.res_save is not None:
            if not os.path.exists(os.path.dirname(args.res_save)):
                os.makedirs(os.path.dirname(args.res_save))
            with open(args.res_save, "w") as wf:
...
def get_swirl_multi_work_sto_res():
    parser = swirl_com.get_parser()
    args = parser.parse_args()
    ...
    exp_dir = "/data1/wz/index/index_eab/eab_algo/swirl_selection/exp_res_sto"
    ...
    for agent in ["dqn"]:
        for sto in ["sto100", "sto300", "sto700", "sto900"]:
            ...
            args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_sto/" \
                            f"tpch_skew_1gb_template_18_multi_work_{sto}_index_{agent}.json"
            if args.res_save is not None:
                if not os.path.exists(os.path.dirname(args.res_save)):
                    os.makedirs(os.path.dirname(args.res_save))
                with open(args.res_save, "w") as wf:
...
def get_mcts_multi_work_res():
    ...
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_level/" \
                    f"tpch_skew_1gb_template_18_multi_work_index_mcts.json"
    ...
    args.db_file = "/data1/wz/index/index_eab/eab_data/db_info_conf/local_db103_tpch_skew_1gb.conf"
    args.schema_file = "/data1/wz/index/index_eab/eab_data/db_info_conf/schema_tpch_1gb.json"
...
def get_mab_multi_work_res():
    ...
    #                 f"tpch_1gb_template_18_multi_work_index{start * gap}-{end * gap}.json"
    args.res_save = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/work_level/" \
                    f"tpch_skew_1gb_template_18_multi_work_index_mab.json"
...
def remove_step_info(res_id):
    res_dir = f"/data1/wz/index/index_eab/eab_olap/bench_result/tpch_skew/{res_id}"
    for file in tqdm(os.listdir(res_dir)):
        if "tpch_skew_1gb_template_18" not in file or "_bak" in file:
            continue
...
if __name__ == "__main__":
    ...
    res_id = "work_permutation"  # work_level, work_shift, work_vol, work_permutation
    # get_swirl_multi_work_res(exp_id, res_id)
    # get_swirl_multi_work_sto_res()

    # get_mcts_multi_work_res()
    get_mab_multi_work_res()

    # work_num, work_freq, work_sto, work_vol, work_shift, work_permutation
