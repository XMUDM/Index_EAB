...
class Table:
    def __init__(self, name, row=-1, weight=-1):
...
class Query:
    def __init__(self, query_id, query_text, columns=None, frequency=1,
                 utility=-1, feature=-1, influence=-1, weight=-1):
        self.nr = query_id
...
class Workload:
    def __init__(self, queries, feature=-1):
...
@total_ordering
class Index:
    def __init__(self, columns, table, estimated_size=None):
        if len(columns) == 0:
